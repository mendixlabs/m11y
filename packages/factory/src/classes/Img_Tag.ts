...
import { ErrorEnum, ImgTagErrorEnum } from "../utils/types";
import { _descriptions } from "../utils/helpers";
export class Img_TagErrorList extends ErrorWarning {
    /**
    ...
    private _scanAllErrors() {
        for (let index = 0; index < this.allTags.length; index++) {
            const tag = this.allTags[index] as HTMLImageElement;
            if (tag) {
                if (tag.alt) {
                    this._pushErrorToErrorList({
                        tag,
                        errorType: ErrorEnum.Error,
                        errorEnumToTarget: ImgTagErrorEnum.ALT_TEXT,
                        errorDescription: _descriptions(
                            ImgTagErrorEnum.ALT_TEXT
                        )
                    });
                }
            }
